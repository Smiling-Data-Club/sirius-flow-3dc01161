...
import { GraduationCap, Briefcase, Send, ArrowRight } from "lucide-react";
...
const JobCard = ({ job, onApply }: { job: JobItem; onApply: (title: string) => void }) => (
  <article className="bg-card rounded-2xl shadow-[0px_20px_40px_rgba(25,28,30,0.06)] p-6 folded-corner transition-transform hover:scale-[1.02] flex flex-col">
    ...
    {job.detail && <p className="text-sm text-muted-foreground mt-2">{job.detail}</p>}
    <button
      onClick={() => onApply(job.title)}
      className="mt-6 inline-flex items-center gap-2 text-primary font-bold text-sm group/btn self-start"
    >
      Jetzt bewerben
        ...
        <SectionReveal stagger>
          ...
          <div className="reveal mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {ausbildung.map((job) => (
                <JobCard key={job.title} job={job} onApply={setActiveJob} />
              ))}
            </div>
          </div>
